...
class PouringEnvBase(gym.Env):
    ...
    def __init__(
        ...
        if self.use_fill_limit:
            ...
            self.max_fill = self.max_particles_cup
    ...
    def __observe(self):
        ...
        max_distance = np.linalg.norm(
            np.array([self.movement_bounds[1], self.movement_bounds[1], self.movement_bounds[1]])
            - np.array([self.movement_bounds[0], self.movement_bounds[0], self.movement_bounds[0]])
        )
    ...
    def _calc_reward(self, n_particles_cup, n_particles_spilled, jerk, action_magnitude):
        ...
        if self.use_fill_limit:
            min_reward = 0
            max_reward = max(
                (((self.max_fill - 0) / self.max_particles) ** 2),
                (((self.max_fill - self.max_particles_cup) / self.max_particles) ** 2),
            )
    ...
    def reset(self, options=None, seed=None):
        ...
        if (
            options is not None and "cleanup" in options.keys()
        ):  # required when opening and closing multiple environments in one script, otherwise the remaining simulation will cause a segmentation fault
            return None
